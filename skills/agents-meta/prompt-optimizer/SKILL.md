...
| Testing | /test-coverage | tdd-workflow, playwright-qa | tdd-guide, e2e-runner |
...
|--------|--------------|
| New Feature | `/plan [feature]. Implement via tdd-workflow. /code-review. /verify.` |
| Bug Fix | `Write failing test for [bug] (tdd-workflow). Fix to green. /verify.` |
| Refactor | `/refactor-clean [scope]. /code-review. /verify.` |
| Research | `Use search-first skill for [topic]. /plan based on findings.` |
| Testing | `tdd-workflow for [module]. playwright-qa (Part B) for critical flows. /test-coverage.` |
...
2. 测试先行 (tdd-workflow skill)：编写登录表单的单元测试和认证流程的集成测试
3. 实现登录页面和认证逻辑
4. /code-review 审查实现
...
2. Test-first via `tdd-workflow` skill — write table-driven tests for success, validation failure, auth failure, not-found
