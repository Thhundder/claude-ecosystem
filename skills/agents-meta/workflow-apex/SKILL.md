---
name: apex
description: "Use for a full structured implementation workflow (Analyze → Plan → Execute → eXamine) with parallel sub-agents, self-validation, and optional adversarial review. Supports autonomous mode, Agent Teams parallel execution, branch+PR automation, and task resumption. TRIGGER: 'implement X', 'build Y feature', '/apex', multi-file implementations, features that benefit from phased execution, when parallel sub-agent exploration accelerates context gathering. SKIP: single-file trivial edits, pure planning (use planner agent), pure debugging (use workflow-debug), pure review (use code-review-standards), docs-only changes."
