...
1. **RFC decomposition** — use pattern 6 (Ralphinho / RFC-Driven DAG)
2. **Quality gates** — `plankton-code-quality` skill + `/quality-gate` command
3. **Eval loop** — `eval-harness` skill for pass@k and regression tracking
4. **Session persistence** — pattern 2 (NanoClaw REPL)
...
When a loop goes off the rails:

- Freeze the loop (stop the cron / kill the dispatcher)
- Run `/harness-audit` to surface configuration issues
- Reduce scope to the failing unit only
