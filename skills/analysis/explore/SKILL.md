---
name: explore
description: "Use for FAST throwaway exploration — codebase lookups, library docs, or web research — when you need context quickly without polluting the main session (runs in a forked context with Haiku). TRIGGER: 'how does X work', quick library API lookup, 'where is Y handled', single-file investigation, broad keyword search, /explore command. SKIP: deep multi-source research (use deep-research), full project onboarding (use codebase-onboarding), tasks that require persistent context, implementation work."
argument-hint: <query>
context: fork
agent: haiku
