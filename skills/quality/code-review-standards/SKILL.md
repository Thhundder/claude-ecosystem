...
description: "Use when reviewing code, judging finding severity, routing to reviewer agents, or deciding merge approval. TRIGGER: 'review', 'PR review', 'code review', pre-commit/pre-merge checks, security-sensitive code touched (auth/payments/user-data/secrets), deciding block vs warn vs info, just wrote or modified code. SKIP: writing new code from scratch (use tdd-workflow), planning or architecture (use planner/architect), unrelated debugging, single-file trivial edits."
origin: ECC
---
...
| `cpp-reviewer` | C++ specific smells, memory safety, modern idioms |
| `performance-optimizer` | Hot paths, N+1 queries, latency-sensitive code |
| `refactor-cleaner` | Dead code, unused exports, duplicates |
| `silent-failure-hunter` | Code that catches errors silently or has hidden failure modes |
| `tdd-guide` | Enforces write-tests-first on new features and bug fixes |
