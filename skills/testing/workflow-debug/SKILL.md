---
name: debug
description: "Use for a structured 5-step debug workflow — reproduce → hypothesize → propose solutions → fix → multi-layer verify (static/build/runtime/user). Includes a log-instrumentation technique when reproduction fails. Auto mode available. TRIGGER: '/debug', 'fix this bug', 'crashes on X', 'returns wrong result', intermittent failure, production incident triage, stuck on a test failure. SKIP: hunting silent swallowed errors (use silent-failure-hunter agent), planning features, code reviews, systemic refactors, clear one-liner typos."
